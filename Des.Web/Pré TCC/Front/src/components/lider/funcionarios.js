...
export default function Funcionarios(props) {
  ...
  return (
    <>
      {mostrarModalTabela && (
        <ModalCentralizado>
          ...
          <ContainerPesquisa>
            ...
            <BotaoEspacado>
              <BotaoAcao onClick={() => abrirModal('adicionar', '')}>
                Adicionar
          ...
          <TabelaEstilizada>
            <Table striped hover>
              ...
              <tbody>
                {props.data.map((report, reportIndex) => (
                  <tr key={reportIndex}>
                    {props.keys.map((key, keyIndex) => (
                      props.hide[keyIndex]
                        ? <td className="d-none d-md-table-cell" key={keyIndex}>{report[key]}</td>
                        : <td key={keyIndex}>{report[key]}</td>
                    ))}
        ...
        <ModalFundoTransparente>
          <ModalCentralizado>
            ...
            <FormularioModal>
              ...
              <InputModal type="text" placeholder="Digite o nome" />
      ...
      {mostrarModalRemover && (
        alert("Confirma saída?")
      )}

      {mostrarModalAtualizar && (
        <ModalFundoTransparente>
          <ModalCentralizado>
            <BotaoFechar onClick={() => { setMostrarModalTabela(false);}}>
              <svg xmlns="http://www.w3.org/2000/svg" height="30px" viewBox="0 -960 960 960" width="30px" fill="#5c4033">
                <path d="m256-200-56-56 224-224-224-224 56-56 224 224 224-224 56 56-224 224 224 224-56 56-224-224-224 224Z" />
              </svg>
            </BotaoFechar>
